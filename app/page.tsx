...
interface Course { day: number; name: string; tees: string; slope: number; cr: number | null; par: number; holes: Hole[]; }
interface Pairing {
...
const DAY_FMT: Record<number, string> = { 1: '2-ball scramble', 2: 'Fourball', 3: 'Singles' };
...
function CoursesStep({ courses, setCourses }: { courses: Course[]; setCourses: (c: Course[]) => void }) {
  ...
  const updC = (f: string, v: string | number) => setCourses(courses.map(x => x.day === ad ? { ...x, [f]: v } : x));
  const updH = (i: number, f: string, v: number) => {
    const holes = c.holes.map((h, j): Hole => j === i ? { ...h, [f]: v } : h);
    setCourses(courses.map(x => x.day === ad ? { ...x, holes } : x));
  };

  return (
    <div>
      <h2 style={{ fontSize: 18, fontWeight: 500, marginBottom: 4 }}>Course details</h2>
      <p style={{ fontSize: 13, color: '#888', marginBottom: '1rem' }}>Can be updated before each day. All figures are on the club scorecard.</p>
      <div style={{ display: 'flex', gap: 6, marginBottom: '1rem' }}>
        {[1, 2, 3].map(d => (
          <button key={d} onClick={() => { setAd(d); setShowH(false); }} style={{ padding: '5px 13px', borderRadius: 999, fontSize: 12, cursor: 'pointer', border: ad === d ? 'none' : '1px solid #d0d0cc', background: ad === d ? '#111' : 'transparent', color: ad === d ? '#fff' : '#888' }}>
            Day {d}
          </button>
        ))}
      </div>
      <div style={S.card}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 8 }}>
          <div><label style={S.lbl}>Course name</label><input style={S.inp} value={c.name} placeholder="e.g. Wentworth" onChange={e => updC('name', e.target.value)} /></div>
          <div><label style={S.lbl}>Tees</label><input style={S.inp} value={c.tees} placeholder="e.g. Yellow" onChange={e => updC('tees', e.target.value)} /></div>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
          <div><label style={S.lbl}>Slope</label><input style={{ ...S.inp, textAlign: 'center' }} type="number" value={c.slope} onChange={e => updC('slope', +e.target.value)} /></div>
          <div><label style={S.lbl}>Course rating</label><input style={{ ...S.inp, textAlign: 'center' }} type="number" step="0.1" value={c.cr ?? ''} placeholder="72.0" onChange={e => updC('cr', e.target.value)} /></div>
          <div><label style={S.lbl}>Par</label><input style={{ ...S.inp, textAlign: 'center' }} type="number" value={c.par} onChange={e => updC('par', +e.target.value)} /></div>
        </div>
      </div>
      <button onClick={() => setShowH(!showH)} style={{ fontSize: 12, color: '#888', background: 'none', border: 'none', cursor: 'pointer', padding: '4px 0', marginBottom: '0.5rem' }}>
        {showH ? '▲ Hide hole data' : '▼ Set hole par & stroke index'}
      </button>
      {showH && (
          ...
          <table style={{ borderCollapse: 'collapse', fontSize: 12 }}>
            <thead>
              <tr>
                ...
                {c.holes.map((_, i) => <th key={i} style={{ padding: '3px 4px', color: '#aaa', fontWeight: 400, textAlign: 'center', minWidth: 36 }}>{i + 1}</th>)}
              </tr>
            </thead>
            <tbody>
              <tr>
                <td style={{ padding: '3px 8px', color: '#aaa', fontSize: 11, whiteSpace: 'nowrap' }}>Par</td>
                {c.holes.map((hole, i) => <td key={i} style={{ padding: 2 }}><input type="number" min={3} max={5} value={hole.par} onChange={e => updH(i, 'par', +e.target.value)} style={S.sm} /></td>)}
              </tr>
              <tr>
                <td style={{ padding: '3px 8px', color: '#aaa', fontSize: 11, whiteSpace: 'nowrap' }}>S.I.</td>
                {c.holes.map((hole, i) => <td key={i} style={{ padding: 2 }}><input type="number" min={1} max={18} value={hole.si} onChange={e => updH(i, 'si', +e.target.value)} style={S.sm} /></td>)}
              </tr>
            </tbody>
          </table>
        </div>
...
// ── Setup flow ────────────────────────────────────────────
function Setup({ onBack, onDone, initPlayers, initCourses }: { onBack: () => void; onDone: () => void; initPlayers: Player[]; initCourses: Course[] }) {
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: '1.5rem' }}>
        {(['players', 'courses'] as const).map((s, i) => (
          <>
            <div key={s} onClick={() => i === 0 && step === 'courses' && setStep('players')} style={{ display: 'flex', alignItems: 'center', gap: 5, cursor: i === 0 && step === 'courses' ? 'pointer' : 'default', opacity: i === 1 && step === 'players' ? 0.4 : 1 }}>
              <div style={{ width: 22, height: 22, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 500, flexShrink: 0, background: (i === 0 && step === 'courses') ? '#1D9E75' : (i === 0 && step === 'players') || (i === 1 && step === 'courses') ? '#111' : '#f0f0ec', color: (i === 0 && step === 'courses') ? '#fff' : (i === 0 && step === 'players') || (i === 1 && step === 'courses') ? '#fff' : '#aaa' }}>
                {i === 0 && step === 'courses' ? '✓' : i + 1}
              </div>
              <span style={{ fontSize: 12, whiteSpace: 'nowrap', color: (i === 0 && step === 'players') || (i === 1 && step === 'courses') ? '#111' : '#aaa' }}>{['Players', 'Courses'][i]}</span>
            </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: '1.5rem', justifyContent: 'space-between' }}>
        <div>{step === 'courses' && <Btn label="← Back" onClick={() => setStep('players')} />}</div>
        {step === 'players'
          ? <Btn label="Next →" primary onClick={() => setStep('courses')} />
          : <Btn label={saving ? 'Saving…' : 'Start tournament →'} primary onClick={save} disabled={saving} />
        }
      </div>
    </div>
  );
        ...
        <table style={{ borderCollapse: 'collapse', fontSize: 12, minWidth: 560 }}>
          <thead>
            <tr>
              ...
              {course.holes.map((_, i) => <th key={i} style={{ ...hs, minWidth: 34 }}>{i + 1}</th>)}
              <th style={{ ...hs, minWidth: 40 }}>Save</th>
            </tr>
            <tr>
              <td style={{ ...hs, textAlign: 'left', fontWeight: 400 }}>Par</td>
              {course.holes.map((hole, i) => <td key={i} style={hs}>{hole.par}</td>)}
              <td />
            </tr>
            <tr>
              <td style={{ ...hs, textAlign: 'left', fontWeight: 400 }}>S.I.</td>
              {course.holes.map((hole, i) => <td key={i} style={hs}>{hole.si}</td>)}
              <td />
            </tr>
          </thead>
          <tbody>
...
function Scorecard({ day, pid, players, course, scores, onBack }: { day: number; pid: string; players: Player[]; course: Course; scores: Record<string, (number | null)[]>; onBack: () => void }) {
  ...
  const phVal = player?.hi !== null && course.cr ? playerPH(player, course) : null;
    ...
    <div>
      ...
      <div style={{ ...S.card, padding: '0.5rem', overflowX: 'auto' }}>
        <table style={{ borderCollapse: 'collapse', fontSize: 12, minWidth: 520 }}>
          <thead>
            <tr><th style={{ ...hs, textAlign: 'left', width: 60 }}>Hole</th>{course.holes.map((_, i) => <th key={i} style={{ ...hs, minWidth: 32 }}>{i + 1}</th>)}</tr>
            <tr><td style={{ ...hs, textAlign: 'left', fontWeight: 400 }}>Par</td>{course.holes.map((h, i) => <td key={i} style={hs}>{h.par}</td>)}</tr>
            <tr><td style={{ ...hs, textAlign: 'left', fontWeight: 400 }}>S.I.</td>{course.holes.map((h, i) => <td key={i} style={hs}>{h.si}</td>)}</tr>
            {phVal !== null && <tr><td style={{ ...hs, textAlign: 'left', fontWeight: 400 }}>Shots</td>{course.holes.map((h, i) => <td key={i} style={hs}>{shotsOnHole(phVal, h.si) || ''}</td>)}</tr>}
          </thead>
          <tbody>
            <tr>
            ...
            {phVal !== null && (
              <tr>
                ...
                {course.holes.map((h, i) => { const s = raw[i]; return <td key={i} style={{ ...cs, color: '#888' }}>{s ? s - shotsOnHole(phVal, h.si) : ''}</td>; })}
              </tr>
            )}
          </tbody>
        </table>
      ...
      <div style={{ ...S.card, background: '#f4f4f0', border: 'none', padding: '0.75rem 1rem', marginBottom: '0.75rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 8 }}>
          <div>
            ...
            {course.tees && <div style={{ fontSize: 12, color: '#888', marginTop: 2 }}>{course.tees} tees · Par {course.par}</div>}
          </div>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: 11, color: '#aaa' }}>Slope / CR</div>
            <div style={{ fontSize: 13, fontWeight: 500 }}>{course.slope} / {course.cr ?? '—'}</div>
          </div>
        </div>
      </div>
...
export default function App() {
  ...
  const navItems = [
    { k: 'home', l: 'Home' },
    ...(setupDone ? [{ k: 'day1', l: 'Day 1' }, { k: 'day2', l: 'Day 2' }, { k: 'day3', l: 'Day 3' }, { k: 'scores', l: 'Scores' }] : []),
  ];
  ...
  return (
    <div style={{ maxWidth: 680, margin: '0 auto', padding: '0 1rem 5rem' }}>
      ...
      {nav === 'setup' && <Setup onBack={() => setNav('home')} onDone={() => { setSetupDone(true); loadAll(); setNav('day1'); }} initPlayers={players} initCourses={courses} />}
