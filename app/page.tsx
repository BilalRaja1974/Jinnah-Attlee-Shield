    ...
    return (
      <div>
        ...
        <div style={{textAlign:'center', padding:'2.5rem 1rem 2rem', background:`linear-gradient(160deg, ${C.dark} 0%, #1a2744 100%)`, margin:'-1rem -1rem 1.25rem', borderRadius:'0 0 24px 24px'}}>
          ...
          {activeYear && <div style={{fontSize:12,color:'#9CA3AF',marginTop:12}}>{activeYear} · 3-day team match play · 20 points</div>}
        </div>
        ...
        <div style={{background:C.dark,borderRadius:12,padding:'1rem',marginBottom:'1rem',color:C.white}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
            <div>
              ...
              {tee.name&&<div style={{fontSize:12,color:'#9CA3AF',marginTop:2}}>{tee.name} tees · Par {tee.par}</div>}
            </div>
            <div style={{textAlign:'right'}}>
              <div style={{fontSize:10,color:'#6B7280'}}>Slope / CR</div>
      ...
      <div>
        ...
        {day===1&&(
          <div style={{...card,background:C.dark,border:'none',marginBottom:'0.75rem'}}>
            ...
            <div style={{fontSize:11,color:'#6B7280',marginBottom:10}}>Each player needs min. 7 drives &amp; 5 second shots. Par 3 drives count. Par 3 second shots don't count.</div>
          ...
          return (
            <div key={label} style={{...card,padding:0,overflow:'hidden',marginBottom:'1rem'}}>
              <div style={{background:C.dark,padding:'8px 12px',display:'grid',
                ...
                {ph!==null&&<div style={{fontSize:10,color:'#6B7280',textAlign:'center'}}>Net</div>}
              </div>
              {sHoles.map((hole,idx)=>{
                const hi=start+idx;
                  ...
                  <div key={hi} style={{display:'grid',
                    ...
                    <div style={{display:'flex',justifyContent:'center'}}>
                      {s!=null
                        ...
                        :<div style={{width:36,height:36,display:'flex',alignItems:'center',justifyContent:'center',borderRadius:'6px',border:`1.5px solid ${C.border}`,color:'#D1D5DB',fontSize:12}}>—</div>
                      }
                    </div>
  ...
  function Leaderboard({players,courses,pairings,scores,onEndTournament}: {
    ...
    return (
      <div>
        {/* Overall scoreboard */}
        <div style={{background:C.dark,borderRadius:16,padding:'1.5rem',marginBottom:'1rem',textAlign:'center'}}>
          <div style={{fontSize:11,color:C.gold,fontWeight:700,letterSpacing:'0.1em',marginBottom:12}}>JINNAH-ATTLEE SHIELD · 20 POINTS</div>
          <div style={{display:'grid',gridTemplateColumns:'1fr 48px 1fr',gap:8,alignItems:'center',marginBottom:12}}>
            <div>
              <div style={{fontSize:12,color:C.pakGreen,fontWeight:700,marginBottom:6}}>{TNAME.A}</div>
              <div style={{fontSize:64,fontWeight:800,color:lead==='A'?C.pakGreen:'#9CA3AF',lineHeight:1}}>{fmtPt(totA)}</div>
            </div>
            <div style={{fontSize:20,color:'#4B5563'}}>–</div>
            <div>
              <div style={{fontSize:12,color:'#4A90D9',fontWeight:700,marginBottom:6}}>{TNAME.B}</div>
              <div style={{fontSize:64,fontWeight:800,color:lead==='B'?'#4A90D9':'#9CA3AF',lineHeight:1}}>{fmtPt(totB)}</div>
            </div>
          </div>
          {lead&&<div style={{fontSize:14,fontWeight:700,color:lead==='A'?C.pakGreen:'#4A90D9',background:lead==='A'?C.pakLight:C.engLight,display:'inline-block',padding:'6px 16px',borderRadius:999}}>
            {TNAME[lead]} lead{Math.abs(totA-totB)===1?'s':''} by {fmtPt(Math.abs(totA-totB))} point{Math.abs(totA-totB)===1?'':'s'}
          </div>}
          {!lead&&(totA+totB)>0&&<div style={{fontSize:14,color:'#9CA3AF'}}>Level</div>}
        </div>
    ...
    return (
      <div>
        <div style={{background:C.dark,borderRadius:12,padding:'1rem',marginBottom:'1rem',color:C.white}}>
          ...
          <div style={{fontSize:17,fontWeight:700}}>{course.name||'Course TBC'}</div>
          {tee.name&&<div style={{fontSize:12,color:'#9CA3AF',marginTop:2}}>{tee.name} tees · Par {tee.par} · Slope {tee.slope} / CR {tee.cr}</div>}
    ...
    return (
      <div style={{maxWidth:480,margin:'0 auto',paddingBottom:'5rem'}}>
          ...
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
            ...
            {setupDone&&(
              <div style={{display:'flex',gap:2}}>
                {DAYS.map(d=>(
                  <button key={d} onClick={()=>{setActiveDay(d);setNav('today');}}
                    style={{padding:'5px 10px',borderRadius:8,fontSize:11,fontWeight:600,cursor:'pointer',
                      border:nav==='today'&&activeDay===d?'none':'1px solid rgba(255,255,255,0.15)',
                      background:nav==='today'&&activeDay===d?C.gold:'transparent',
                      color:nav==='today'&&activeDay===d?C.dark:'#9CA3AF'}}>
                    D{d}
                  </button>
                ))}
              </div>
        ...
        <div style={{padding:'0 1rem'}}>
          ...
          {nav==='home'&&<Home setupDone={setupDone} activeYear={activeYear} allYears={allYears} completedTournaments={completedTournaments} onStartYear={startYear} onSwitchYear={switchYear} onSetup={()=>setNav('setup')} onPlay={()=>{setActiveDay(1);setNav('today');}}/>}
        ...
        <div style={{position:'fixed',bottom:0,left:0,right:0,background:C.white,borderTop:`1px solid ${C.border}`,display:'flex',zIndex:20,maxWidth:480,margin:'0 auto'}}>
          {[
            ...
            {k:'scores' as const,icon:'◉',label:'Scores'},
